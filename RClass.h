...
{
	sf::Vector2f spaceStep = {subSpace.width / static_cast<float>(image.getSize().x),subSpace.height / static_cast<float>(image.getSize().y)};
...
void RClass::SaveImageToFile(const std::string &filename)
{
